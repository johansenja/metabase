...
(defn render-multiple-lab-chart
  ...
  (let [viz-settings  (merge viz-settings (:visualization_settings dashcard))
        ...
        row-seqs      (for [[row-seq [x-rowfn y-rowfn]] (map vector row-seqs rowfns)]
...
(defn- multiple-scalar-series
  [_type joined-rows _x-cols _y-cols _viz-settings]
...
(defn render-multiple-scalars
  ...
  (let [viz-settings (merge viz-settings (:visualization_settings dashcard))
        ...
        series       (multiple-scalar-series :bar (mapv vector x-rows (flatten y-rows)) x-cols y-cols viz-settings)
